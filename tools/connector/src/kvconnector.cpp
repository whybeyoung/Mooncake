...
KVConnector::KVConnector() {
    ...
    VLOG(1) << "KVConnector created";
}

KVConnector::~KVConnector() {
    VLOG(1) << "KVConnector destroying";
...
int KVConnector::setup(const std::string& local_hostname,
                       ...
                       wrapperMetrics) {
    VLOG(1) << "KVConnector::setup called, local_hostname=" << local_hostname
            << ", protocol=" << protocol
    ...
    VLOG(1) << "KVConnector::setup completed successfully";
    return 0;
}
...
int KVConnector::put(const std::string& key, const std::string& value,
                     size_t replica_num,
                     const std::string& preferred_segment) {
    VLOG(2) << "KVConnector::put called, key=" << key
            << ", value_size=" << value.size()
            << ", replica_num=" << replica_num;
    auto* state = GetState(this);
    if (state == nullptr || !state->dummy_client) {
        LOG(ERROR) << "KVConnector::put: state or dummy_client is null";
        return 1;
    }
    if (key.empty()) {
        LOG(ERROR) << "KVConnector::put: key is empty";
        return 1;
    }
    const auto config =
    ...
    if (rc != 0) {
        VLOG(1) << "KVConnector::put failed, rc=" << rc;
    }
    return rc == 0 ? 0 : 1;
}

bytes KVConnector::get(const std::string& key) {
    VLOG(2) << "KVConnector::get called, key=" << key;
    auto* state = GetState(this);
    if (state == nullptr || !state->dummy_client || key.empty()) {
        if (state == nullptr || !state->dummy_client) {
            VLOG(1) << "KVConnector::get: state or dummy_client is null";
        }
        return bytes();
    }
    ...
    if (replica_descriptors.empty()) {
        VLOG(1) << "KVConnector::get: no replica descriptors found";
        return bytes();
    }

    auto handle = state->dummy_client->get_buffer(key);
    if (!handle || handle->ptr() == nullptr || handle->size() == 0) {
        VLOG(1) << "KVConnector::get: invalid buffer handle";
        return bytes();
    }
