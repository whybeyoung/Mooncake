...
MasterService::MasterService(const MasterServiceConfig& config)
    : default_kv_lease_ttl_(config.default_kv_lease_ttl),
      ...
      enable_group_grant_lease_(config.enable_group_grant_lease),
...
void MasterService::GrantLeaseToGroup(const std::string& key) {
    if (!enable_group_grant_lease_) {
        return;
    }

    auto group_members = GetGroupMembers(key);
